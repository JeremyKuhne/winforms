...
namespace System.Windows.Forms.ComponentModel.Com2Interop
{
    internal unsafe class Com2IVsPerPropertyBrowsingHandler : Com2ExtendedBrowsingHandler
    {
        ...
        {
            using var browsing = ComHelpers.GetComScope<IVsPerPropertyBrowsing>(propDesc.TargetObject, out bool success);
            if (success)
            {
                BOOL pfDisplay = false;
                HRESULT hr = browsing.Value->DisplayChildProperties(propDesc.DISPID, &pfDisplay);
                if (hr == HRESULT.S_OK)
                {
                    return pfDisplay;
                }
        ...
        {
            using var browsing = ComHelpers.GetComScope<IVsPerPropertyBrowsing>(sender.TargetObject, out bool success);
            if (!success)
            {
                return;
            }
            ...
            HRESULT hr = browsing.Value->GetLocalizedPropertyInfo(sender.DISPID, PInvoke.GetThreadLocale(), null, &helpString);
        ...
        private void OnGetDynamicAttributes(Com2PropertyDescriptor sender, GetAttributesEvent attrEvent)
        {
            ...
            if (!success)
            {
                Debug.Assert(sender.TargetObject is null, $"Object is not {Interface.Name}!");
                return;
            }
            ...
            if (sender.CanShow)
            {
                ...
                if (browsing.Value->HideProperty(sender.DISPID, &hide).Succeeded)
                {
