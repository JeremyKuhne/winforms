...
#pragma warning disable IDE0051 // Remove unused private members
    ...
    private static void ThrowNegative<T>(T value, string? paramName) =>
        throw new ArgumentOutOfRangeException(
            paramName,
            value,
            string.Format(SR.ArgumentOutOfRange_Generic_MustBeNonNegative, Value.Create(paramName), Value.Create(value)));
    ...
    throw new ArgumentOutOfRangeException(
        paramName,
        value,
        string.Format(SR.ArgumentOutOfRange_Generic_MustBeGreaterOrEqual, Value.Create(paramName), Value.Create(value), Value.Create(other)));
