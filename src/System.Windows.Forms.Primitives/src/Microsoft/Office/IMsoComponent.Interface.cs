...
internal unsafe partial struct IMsoComponent
{
    /// <summary>
    ...
    /// <remarks>
    ///  <para>
    ///   <see cref="PInvoke.WM_MOUSEACTIVATE"/> Note (for top level components and host)
    ///  </para>
    ///  <para>
    ...
    ///   <see cref="IMsoComponentManager.FGetActiveComponent(msogac, IMsoComponent**, MSOCRINFO*, uint)"/>.
    ///  </para>
    ///  <para>
    ...
    internal unsafe interface Interface
    {
        ...
        ///  as TranslateAccelerator or <see cref="PInvoke.IsDialogMessage(HWND, MSG*)"/>, or take some other action.
        /// </summary>
        /// <returns>
        ...
        /// <remarks>
        ///  <para>
        ///   Note: If n calls are made with <see cref="BOOL.TRUE"/> <paramref name="fEnter"/>, component should consider
        ///   the state to be in effect until n calls are made with <see cref="BOOL.FALSE"/> <paramref name="fEnter"/>.
        ///  </para>
        ///  <para>
        ///   Note: Components should be aware that it is possible for this method to be called with
        ///   <see cref="BOOL.FALSE"/> <paramref name="fEnter"/> more times than it was called with <see cref="BOOL.TRUE"/>
        ///   <paramref name="fEnter"/> (so, for example, if component is maintaining a state counter
        ///   incremented when this method is called with <see cref="BOOL.TRUE"/> <paramref name="fEnter"/>,
        ///   decremented when called with <see cref="BOOL.FALSE"/> <paramref name="fEnter"/>), the counter should not
        ///   be decremented for <see cref="BOOL.FALSE"/> <paramref name="fEnter"/> if it is already at zero.)
        ///  </para>
        /// </remarks>
        [PreserveSig]
        void OnEnterState(
        ...
        ///   by appropriately processing <see cref="PInvoke.WM_MOUSEACTIVATE"/> (but see WM_MOUSEACTIVATE NOTE, above).
        ...
        ///   <paramref name="fSameComponent"/> is <see cref="BOOL.TRUE"/> if <paramref name="pic"/> is the same
        ///   component as the callee of this method, and <paramref name="pcrinfo"/> is the
        ///   reg info of <paramref name="pic"/>.
        ///  </para>
        ///  <para>
        ///   If null and <paramref name="fHostIsActivating"/> is <see cref="BOOL.TRUE"/>, then the host is the
        ///   object being activated, and <paramref name="pchostinfo"/> is its host info.
        ///  </para>
        ///  <para>
        ///   If null and <paramref name="fHostIsActivating"/> is <see cref="BOOL.FALSE"/>, then there is no
        ...
        ///   via <see cref="IMsoComponentManager.FUpdateComponentRegistration(nuint, MSOCRINFO*)"/>.
        ///  </para>
        ///  <para>
        ...
        ///   (via <see cref="PInvoke.PeekMessage(MSG*, HWND, uint, uint, PEEK_MESSAGE_REMOVE_TYPE)"/>)
        ...
        /// </remarks>
        /// <param name="uReason">
        ///  Reason that was passed to <see cref="IMsoComponentManager.FPushMessageLoop(nuint, msoloop, void*)"/>.
        /// </param>
        /// <param name="pvLoopData">
        ///  Component private data passed to <see cref="IMsoComponentManager.FPushMessageLoop(nuint, msoloop, void*)"/>.
        /// </param>
        /// <returns>
        ///  <para>
        ...
        /// <remarks>
        ///  <para>
        ///   If <paramref name="fPromptUser"/> is <see cref="BOOL.FALSE"/>, component should simply return
        ///   <see cref="BOOL.TRUE"/> if it can terminate, <see cref="BOOL.FALSE"/> otherwise.
        ///  </para>
        ///  <para>
        ///   If <paramref name="fPromptUser"/> is <see cref="BOOL.TRUE"/>, component should return <see cref="BOOL.TRUE"/>
        ///   if it can terminate without prompting the user; otherwise it should prompt the user, either 1.) asking user
        ///   if it can terminate and returning <see cref="BOOL.TRUE"/> or <see cref="BOOL.FALSE"/> appropriately, or 2.)
        ///   giving an indication as to why it cannot terminate and returning <see cref="BOOL.FALSE"/>.
        ///  </para>
        /// </remarks>
        [PreserveSig]
        ...
        /// </summary>
        /// <returns>
        ///  Desired window or <see cref="HWND.Null"/> if no such window exists.
        /// </returns>
