...
internal static partial class PrimitivesPInvokeExtensions
{
    extension(PInvoke)
    {
        /// <inheritdoc cref="PInvoke.HtmlHelp(HWND, string, HTML_HELP_COMMAND, nuint)" />
        internal static unsafe HWND HtmlHelp<T>(T hwndCaller, string? pszFile, HTML_HELP_COMMAND uCommand, nuint dwData)
            where T : IHandle<HWND>
        {
            HWND result = PInvoke.HtmlHelp(hwndCaller.Handle, pszFile, uCommand, dwData);
            ...
            where T : IHandle<HWND>
        {
            fixed (void* d = dwData)
            {
                HWND result = PInvoke.HtmlHelp(hwndCaller.Handle, pszFile, uCommand, (nuint)d);
                GC.KeepAlive(hwndCaller.Wrapper);
                return result;
            }
        }
            ...
            where TData : unmanaged
        {
            fixed (void* v = &dwData)
            {
                HWND result = PInvoke.HtmlHelp(hwndCaller.Handle, pszFile, uCommand, (nuint)v);
                GC.KeepAlive(hwndCaller.Wrapper);
                return result;
            }
        }
