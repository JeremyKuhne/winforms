...
internal static partial class PrimitivesPInvokeExtensions
{
    extension(PInvoke)
    {
        public static HINSTANCE LoadComctl32(string startupPath)
        {
            ...
            if (!string.IsNullOrWhiteSpace(startupPath))
            {
                ...
                if (Path.IsPathFullyQualified(customPath))
                {
                    ...
                    HINSTANCE result = PInvoke.LoadLibraryEx(customPath, 0);
                    if (!result.IsNull)
                    {
                        return result;
                    }
        ...
        {
            if (PInvoke.GetModuleHandle(Libraries.Kernel32).IsNull)
            {
            ...
            HINSTANCE result = PInvoke.LoadLibraryEx(libraryName, LOAD_LIBRARY_FLAGS.LOAD_LIBRARY_SEARCH_SYSTEM32);
            if (!result.IsNull)
            {
                return result;
            }
            ...
            return PInvoke.LoadLibraryEx(libraryName, 0);
        }
    }
}
