...
internal static partial class PrimitivesPInvokeExtensions
{
    extension(PInvoke)
    {
        /// <inheritdoc cref="PInvoke.CreateWindowEx(WINDOW_EX_STYLE, string, string, WINDOW_STYLE, int, int, int, int, HWND, HMENU, HINSTANCE, void*)"/>
            ...
            object? lpParam)
        {
            ...
            fixed (char* wn = lpWindowName)
            {
                ...
                {
                    // No need to marshal.
                    return PInvoke.CreateWindowEx(dwExStyle, cn, wn, dwStyle, X, Y, nWidth, nHeight, hWndParent, hMenu, hInstance, null);
                }
                ...
                if (lpParam is string stringValue)
                {
                    ...
                    {
                        return PInvoke.CreateWindowEx(dwExStyle, cn, wn, dwStyle, X, Y, nWidth, nHeight, hWndParent, hMenu, hInstance, sv);
                    }
                }
                ...
                try
                {
                    ...
                    return PInvoke.CreateWindowEx(dwExStyle, cn, wn, dwStyle, X, Y, nWidth, nHeight, hWndParent, hMenu, hInstance, (void*)native);
                }
                finally
                {
                    if (native != 0)
